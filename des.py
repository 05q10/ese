# ------------------------------------------------------
# Simplified DES Implementation (No Libraries)
# Author: ChatGPT
# ------------------------------------------------------
...
def des_round(L, R, key):
    ...
    sboxed = sbox_substitution(mixed)
    permuted = permute(sboxed, P)
    return R, xor(L, permuted)

# ---------------- DES CORE ---------------------------

def DES(text64, key64, encrypt=True):
    keys = generate_keys(key64)
    if not encrypt:
        keys.reverse()

    text = permute(text64, IP)
    L = text[:32]
    R = text[32:]
...
if __name__ == "__main__":
    plaintext = input("Enter 8-character plaintext: ")[:8].ljust(8)
    key = input("Enter 8-character key: ")[:8].ljust(8)

    bin_plain = str_to_bin(plaintext)
    bin_key = str_to_bin(key)

    encrypted = DES(bin_plain, bin_key, True)
    print("\nEncrypted (binary):", encrypted)

    decrypted = DES(encrypted, bin_key, False)
    print("Decrypted text:", bin_to_str(decrypted))
